...
## Git ワークフロー（このリポジトリの方針・厳守）
- **ブランチを勝手に切らない。コミットは全部 `main` に直接コミットして `main` へ push する。**
  feature ブランチや PR は作らない（ユーザー方針）。
- push のリモートは SSH。鍵指定が要る場合は
  `GIT_SSH_COMMAND="ssh -i ~/.ssh/hiroyuki-s1 -o IdentitiesOnly=yes" git push origin main`。
- コミットは push を頼まれたときに行う。コミットメッセージは Conventional Commits（`feat:`/`fix:` 等）。
  **attribution（Co-Authored-By 等）は付けない**（リポジトリ慣習）。
- ⚠️ **push ≠ 本番反映**（下記 Build/Deploy 参照）。本番公開は人間が手動デプロイした時だけ。
- **push したら必ずステージング URL を提示する**。`main` への push で Cloudflare Pages の
  ステージング環境に自動反映されるので、push 完了の報告に必ず次の URL を添える:
  **https://kami-scale-trainer-staging.pages.dev/** （反映まで数十秒〜数分かかる）。

## Tech Stack
- Vanilla HTML5 + ES2022 modules (フレームワークなし・**ランタイム依存は原則ゼロ**。
  追加は軽さ/寿命/可搬性とのトレードオフを検討のうえ可 — 下記 JavaScript Rules 参照)
...
- External runtime dependencies: **原則なし（思考停止で守るルールではない）**。
  「依存ゼロ」は目的ではなく手段で、本当の目的は **軽さ（初回ロード/SWキャッシュ）・
  寿命（腐らない・破壊的更新に追従し続けない）・可搬性（不具合が自分のコードで完結）**。
  → **目的とのトレードオフを言語化したうえでなら依存追加可**。判断基準:
  - ✅ ビルド時のみ（出荷物に残らない: Vite/Vitest/ESLint）/ 遅延ロードで使う人だけが払う /
    自前実装だと事故る専門領域（例: ニューラル推論）/ 標準に近く枯れている
  - ❌ 起動経路に常時ロード（React/Tonal.js 等の全機能から参照する常駐依存）/
    数百行で自前実装できる小ドメイン / 流行り廃りが速い
  - 入れる場合の鉄則: **コア（指板可視化・度数計算・印刷・チューナー）は依存ゼロを死守**し、
    重い依存は **機能境界の向こうへ動的 import で隔離**（無料/コア体験のバンドル・SWキャッシュを
    太らせない）。例: ニューラル採点の `onnxruntime-web` は有料機能でのみ遅延ロード。
...
### ピッチ検出 (チューナー / 将来のスケール採点) — 低レイテンシ設計
「レスポンスが命」のため検出は **AudioWorklet（オーディオスレッド）** で走る。再利用エンジン化済み。
- **レイテンシの主因は観測窓**（基音1周期は最低限聞かないと F0 が出ない物理下限）。窓は
  `domain/pitch.js` 任せにせず **最低音 minHz の約2.5周期**に右サイズ化（worklet 内）。
- **約12kHz へダウンサンプル**してから YIN（F0≤1.3kHz に 48kHz は過剰）。`domain/dsp/downsample.js`
  は **box長2D・ストライドD の安価な移動平均**（アンチエイリアス）。worklet はこれをストリーミングで実施。
- YIN 差分関数は `detectPitchYIN(buf, sr, { useFFT:true })` で **FFT 自己相関**（`domain/dsp/autocorr.js`、
  O(N log N)）。既定 `useFFT:false`（時間領域）と数値誤差内で一致＝**実行経路=テスト経路**を pitch.test で固定。
...
- **差別化機能（競合が課金で隠す/高級機専用を無料Webで）**:
  ...
  - **スウィートンド/オフセット**(`SWEETENED`): 弦ごとに目標を ±¢ 補正し開放和音の濁りを抑える。
    検出目標 = `targetHz(midi,a4,offsetCents)`。エンジン・指板はそのまま。
  - **ポリフォニック**(`domain/polyphonic.js`): ジャラーンと1回で全弦の±¢を同時表示。完全採譜はせず
    「既知の弦目標近傍のFFTピークを放物線補間」。探索窓は弦間隔の半分未満で干渉なし。worklet poly モードが
    `detectStringOffsets` を回し `{type:'poly',strings}` を post。
  - **ストロボ**(`domain/strobe.js`): 位相を時間積分（`advanceStrobePhase`）し縞の流れで高感度表示。
    合致=静止 / 右=高い・左=低い。
- **3表示モード**(tuner.js): メーター(mono・グラフ) / ストロボ(mono・縞) / ポリ(poly・全弦)。
  チューニング/甘い調弦/モードは localStorage 保存。ノーマル(クロマチック)はポリ不可。
- 検証: `e2e/tuner.test.cjs`（Chromium fake audio capture）が単音/オルタネート(Drop D→D2)/
  甘い調弦(±0→+4¢)/ポリ(和音で全6弦) まで実機検証。和音 WAV は `e2e/gen-tone-wav.cjs` の `writeChordWav`。
- **メインスレッド・フォールバックは持たない**。AudioWorklet 非対応は `isPitchEngineSupported()` で gate し案内表示。
- **★ Vite × AudioWorklet バンドル (CRITICAL・ハマった)**: `new URL(...worklet.js, import.meta.url)` は
  Vite が **verbatim コピー**するだけで依存 import を解決せず、相対 import が実機で 404 になる
  （worklet 内 import はブラウザ差で不安定）。→ `vite.config.js` の `pitchWorkletPlugin` が **esbuild で
  依存込み自己完結 1 ファイル**にバンドルし `public/pitchProcessor.worklet.js`（gitignore 済み・安定URL・
  SW実行時キャッシュ対象）へ出力。数式は domain を import するので**一次ソースは pitch.js のまま**。
  ビルド後は `dist/pitchProcessor.worklet.js` に **runtime import が残っていないこと**を必ず確認。
- 検出は `e2e/tuner.test.cjs`（Chromium fake audio capture に WAV を流す）で E1〜E4/A=442 まで実機検証。

## Architecture Rules

**Module dependency direction:**
```
ui/* → state/store + audio/* + domain/* + config*
audio/* → domain/* のみ (Web Audio 配線層。ui/* は import 禁止)
...
- `audio/*` — Web Audio 配線層 (AudioContext/AudioWorkletNode/Worklet)。DOM 非依存だが
  pure でもない第三層。**domain/* のみ import 可・ui/* は import 禁止**。consumer (ui) が
  生 F0 を購読し、音名判定 (`freqToNote`/`nearestOpenString`) は domain で行う。
