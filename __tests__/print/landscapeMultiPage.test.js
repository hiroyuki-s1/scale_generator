...
describe('横向き印刷用 CSS の構造', () => {
  for (const [cols, rows] of LAYOUT_PRESETS) {
    it(`[${cols}×${rows}] PC orientation=landscape: 単一 @media print に height 182mm + svg.fb max-height mm + title pt`, () => {
      const { layout } = buildPrintCss({ orientation: 'landscape', cols, rows });
      expect(layout).toMatch(/\.print-page-group\s*\{[^}]*height:\s*182mm/);
      expect(layout).toMatch(/svg\.fb\s*\{[^}]*max-height:\s*[\d.]+mm/);
      expect(layout).toMatch(/\.fb-title[^{]*\{[^}]*font-size:\s*[\d.]+pt/);
    });

    it(`[${cols}×${rows}] mobile: landscape orientation media query に height 182mm`, () => {
      ...
      expect(landBlock).toMatch(/\.print-page-group\s*\{[^}]*height:\s*182mm/);
  ...
  it('PC: orientation=portrait 引数では portrait 寸法のみ出力 (両方の混在は viewport-vs-@page 食い違いの原因)', () => {
    ...
    expect(layout).toMatch(/\.print-page-group\s*\{[^}]*height:\s*269mm/);
    expect(layout).not.toMatch(/182mm/);
  });
