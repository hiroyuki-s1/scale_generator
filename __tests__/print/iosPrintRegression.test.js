...
describe('iOS印刷 再発防止 ②b @page: モバイルは orientation media query 両方 / PC は単一', () => {
  // 履歴: モバイルで @page size を片方固定すると、OS印刷シートで横にしたとき
  //   実用紙とCSSの向きがズレ「タイトル1P・スケール2P」に分割された → media query 両方。
  //   一方 PC で media query を使うと viewport(横長)基準で誤評価され portrait 印刷に
  //   landscape @page が当たり用紙はみ出し → PC は orientation 引数で単一ブロック。
  for (const [cols, rows] of LAYOUT_PRESETS) {
    for (const orientation of ORIENTATIONS) {
      const label = `${orientation} ${cols}×${rows}`;
      it(`${label} mobile: @page portrait/landscape を両方出力 (OS切替追従)`, () => {
        const { orient } = buildPrintCss({ orientation, cols, rows, isMobile: true });
        expect(orient).toMatch(/@media print and \(orientation:\s*portrait\)[^}]*@page\s*\{[^}]*210mm\s+297mm/);
        expect(orient).toMatch(/@media print and \(orientation:\s*landscape\)[^}]*@page\s*\{[^}]*297mm\s+210mm/);
      });
      it(`${label} PC: orientation media query を使わず単一 @page (viewport食い違い回避)`, () => {
